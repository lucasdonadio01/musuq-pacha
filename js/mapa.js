(function () {
  ...
  let cercania = 0;
  let filtroElementos = 'todo', solicitarFauna = false;
  let floraMapa = null, viviendasMapa = null, camaraSocial = camara, plantaPendiente = null;
  let estadoSocialAnterior = '';
  ...
  document.getElementById('ver-archivo')?.addEventListener('click', (event) => {
    // Conservar abrir en otra pestaña y el enlace normal si el puente no está disponible.
    if (event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
    const puente = window.MUSUQ_ARCHIVO_TRANSICION;
    const zona = zonaFijada || zonaActiva;
    if (!puente || !zona) return;
    event.preventDefault();
    const activa = arbolVisto && cercania > 0 ? camaraCerca : camara;
    const origen = activa.position.clone(), direccion = activa.getWorldDirection(new THREE.Vector3());
    const centro = new THREE.Vector3(); let cantidad = 0;
    for (let i = 0; i < N; i++) if (C.zonas[i] & (1 << zona.id)) {
      centro.add(new THREE.Vector3(C.x[i], P.z0 + C.nivel[i] * P.paso * relieve + P.elevacionHover, -C.y[i])); cantidad++;
    }
    if (cantidad) centro.divideScalar(cantidad); else centro.copy(origen).addScaledVector(direccion, 300);
    const desplazamiento = centro.sub(origen);
    desplazamiento.addScaledVector(direccion, -desplazamiento.dot(direccion));
    const zoomInicial = activa.zoom;
    viajeArchivo = {camara:activa, origen, desplazamiento, zoomInicial, avance:0};
    reproducir(sonidosVista.entrar, 0, 1);
    puente.salir({url:event.currentTarget.href, pueblo:zona.id, color:colorDeZona(zona),
      progress:avance => { if (viajeArchivo) viajeArchivo.avance = avance; },
      restore:() => { activa.zoom = zoomInicial; activa.updateProjectionMatrix(); viajeArchivo = null; }
    });
  });

  function aplicarViajeArchivo() {
    if (!viajeArchivo) return;
    const v = viajeArchivo;
    v.camara.position.copy(v.origen).addScaledVector(v.desplazamiento, v.avance);
    v.camara.zoom = v.zoomInicial * Math.exp(Math.log(5) * v.avance);
    v.camara.updateProjectionMatrix();
    v.camara.updateMatrixWorld();
  }
  ...
  function fijarZona(zona) {
    viviendasMapa?.cerrar(true);
    floraMapa?.cerrar();
    if (filtroElementos === 'fauna') solicitarFauna = true;
    if (zona !== zonaFijada) {
      arbolFoco = null;
    }
  ...
  lienzo.addEventListener('pointermove', (e) => {
    if(viviendasMapa?.seleccion)return;
    lienzo.classList.toggle('sobre-vivienda',!!viviendasMapa?.hover(e));
  ...
  function detectarZona() {
    if (arbolFoco || cercania > 0 || viviendasMapa?.seleccion) {
      return;
    }
  ...
  lienzo.addEventListener('click', (e) => {
    if (viviendasMapa?.detectar(e)) return;
    if (floraMapa?.detectar(e)) return;
    if(!explorando){
  ...
  function cambiarModo(valor){
    ...
    if (!valor) floraMapa?.cerrar();
    if (!valor) viviendasMapa?.cerrar(true);
  ...
  window.addEventListener('wheel',e=>{
    ...
    if(!explorando||e.target.closest('#selector,#mapa-panel,#carteles-arboles,#vista-arbol,#vista-fauna,#fauna-controles,#header,#mapa-comentarios,#mapa-elementos,#flora-ficha,#vivienda-ficha,.mapa-social,.flora-marcadores')||Math.abs(e.deltaY)<4)return;
    e.preventDefault();
    if(viviendasMapa?.seleccion)return;
    const ahora = performance.now();
    ...
    if(floraMapa?.cerrar()||habitat?.cerrar()){esperaRueda=ahora+900;return;}
    if (arbolFoco) {
      cerrarArbol();
  ...
  lienzo.addEventListener('touchmove',e=>{
    ...
    if(explorando&&inicioTacto!==null&&Math.abs(e.touches[0].clientY-inicioTacto)>45){
      if(!floraMapa?.cerrar()&&!habitat?.cerrar()){if(arbolFoco)cerrarArbol();else if(zonaFijada)fijarZona(null);else cambiarModo(false);}
      inicioTacto=null;
    }
  },{passive:true});
  document.addEventListener('keydown', (e) => {
    if(document.querySelector('dialog[open]'))return;
    if (e.key === 'Escape') {
      if(viviendasMapa?.cerrar()){e.preventDefault();return;}
    ...
    }else if(explorando && ['PageDown','PageUp'].includes(e.key) && !e.target.closest('#selector,#vivienda-ficha')&&!viviendasMapa?.seleccion){
      cambiarModo(false);
    }
  });
  ...
  function cuadroAnimacion(ahora) {
    // El primer timestamp de rAF puede preceder a performance.now() de la carga.
    // No avanzar hacia atrás: contaminaría la ráfaga y el desenfoque con NaN.
    const dt = anterior === null ? 0 : Math.max(0, Math.min(0.05, (ahora - anterior) / 1000));
    anterior = ahora;
    ...
    const rafagaObjetivo = !instantaneo && acercamientoObjetivo === 1 && acercamiento < 1 ? Math.pow(Math.max(0, Math.sin(Math.PI * acercamiento)), 1.5) : 0;
    ...
    actualizarBloqueoHover();
    bosques?.filtrar(viviendasMapa?.seleccion?'todo':filtroElementos);
    bosques?.actualizar(dt,instantaneo,viviendasMapa?.camara||(arbolVisto&&cercania>0?camaraCerca:camara),viviendasMapa?.foco);
    ubicarCamara();
    ...
    // Los sprites del árbol pueden terminar de cargar después de la planta.
    // Conservar el clic hasta que esté listo el entorno de su vista de detalle.
    if(plantaPendiente){
      if(!floraMapa?.seleccion||arbolFoco)plantaPendiente=null;
      else{
        bosques?.filtrar('todo');
        const grupos=gruposZona(),grupo=grupos[0];
        bosques?.filtrar(filtroElementos);
        if(grupo&&matCuadro.uniforms.avanceBrote.value>=1){
          // Esperar a que el brote termine: un grupo incompleto puede ubicar
          // la cámara dentro de los árboles que todavía están apareciendo.
          const firma=[grupos.length,grupo.clave,grupo.n,...[grupo.x,grupo.z,grupo.base,grupo.alto].map(v=>v.toFixed(2))].join(':');
          plantaPendiente.estable=firma===plantaPendiente.firma?plantaPendiente.estable+dt:0;
          plantaPendiente.firma=firma;
          if(instantaneo||plantaPendiente.estable>=.4){abrirArbol(grupo);plantaPendiente=null;}
        }
      }
    }
    ...
    let camaraActiva = enArbol ? camaraCerca : camara;
    ...
    aplicarViajeArchivo();
    camaraSocial = camaraActiva;
    if (solicitarFauna && zonaFijada?.id === 14 && acercamiento > 0.95 && !arbolFoco) {
      bosques?.filtrar('todo');
      const primerGrupo = bosques?.grupos()[0];
      bosques?.filtrar(filtroElementos);
      if (primerGrupo) { solicitarFauna = false; abrirArbol(primerGrupo); }
    }
    if (filtroElementos === 'fauna') heroe.visible = false;
    floraMapa?.actualizar({camara:camaraActiva,explorando,zonaId:zonaFijada?.id,cercania,enfoque,tiempo,instantaneo,base:heroe.position,dir:direccionHeroe,enArbol,dt,brote:matCuadro.uniforms.avanceBrote.value});
    viviendasMapa?.actualizar({explorando,zonaId:zonaFijada?.id,cercania,dt,instantaneo,brote:matCuadro.uniforms.avanceBrote.value,camara:camaraActiva});
    if(viviendasMapa?.camara)camaraActiva=viviendasMapa.camara;
    floraMapa?.aplicarCamara(camaraActiva,dt);
    floraMapa?.actualizarCarteles(camaraActiva,dt,habitat?.enFauna||!!viviendasMapa?.seleccion);
    habitat?.actualizarCarteles(camaraActiva,dt,floraMapa?.enFlora);
    const estadoSocial = [explorando,zonaFijada?.id,!!arbolFoco,filtroElementos,habitat?.seleccion,habitat?.disponible,floraMapa?.seleccion,viviendasMapa?.seleccion,viviendasMapa?.interior].join(':');
    ...
    matHeroe.uniforms.opacidad.value = opacidadHeroe*(1-Math.max(habitat?.zoom||0,floraMapa?.zoom||0));
    ...
    matCielo.uniforms.presencia.value = viviendasMapa?.seleccion?1:THREE.MathUtils.smoothstep(cercania, 0.2, 0.75);
    ...
    const mostrarVista = filtroElementos !== 'fauna' && !habitat?.enFauna && !floraMapa?.enFlora && !!arbolFoco && (!cambioArbol || cambioArbol.t > 0.7) && cercania >= 0.9;
    ...
    const conFoco = !viviendasMapa?.seleccion&&(enfoque > 0.001 || rafaga > 0.001 || rafagaArbol > 0.001 || lejania > 0.001);
    renderer.setRenderTarget(conFoco ? objetivoFoco : null);
    renderer.clear();
    if (enArbol||viviendasMapa?.seleccion) {
      renderer.render(escenaCielo, camaraActiva);
    }
    ...
    const verFronteras = viviendasMapa?.seleccion?0:suaveFronteras * (arbolVisto ? 1 - THREE.MathUtils.smoothstep(cercania, 0, 0.3) : 1);
    ...
    renderer.clearDepth();
    floraMapa?.render(renderer,camaraActiva);
    window.MUSUQ_ESPECIE_CARTEL?.render(renderer,camaraActiva);
    requestAnimationFrame(cuadroAnimacion);
  }

  // Only publish semantic controls and projected coordinates; scene ownership stays here.
  const celdasSociales = new Map();
  function celdaSocial(id,dx=0,dz=0) {
    const clave=id+':'+dx+':'+dz;
    if(celdasSociales.has(clave))return celdasSociales.get(clave);
    const indices=[];let x=0,z=0;
    for(let i=0;i<N;i++)if(C.zonas[i]&(1<<id)){indices.push(i);x+=C.x[i];z-=C.y[i];}
    if(!indices.length)return null;
    x=x/indices.length+dx;z=z/indices.length+dz;
    const elegido=indices.reduce((mejor,i)=>Math.hypot(C.x[i]-x,-C.y[i]-z)<Math.hypot(C.x[mejor]-x,-C.y[mejor]-z)?i:mejor,indices[0]);
    celdasSociales.set(clave,elegido);return elegido;
  }
  const puntoSocial=new THREE.Vector3();
  function posicionSocial(id,dx=0,dz=0){
    const i=celdaSocial(id,dx,dz);
  ...
  window.MUSUQ_MAPA_UI={
    ...
    verPueblo(id){const zona=zonas.find(z=>z.id===id);if(!zona)return;viviendasMapa?.cerrar(true);floraMapa?.cerrar();habitat?.cerrar();cerrarArbol();cambiarModo(true);fijarZona(zona);},
    verEspecie(tipo,id){
    ...
    proyectarZona(id,dx=0,dz=0){
      ...
      const r=lienzo.getBoundingClientRect();
      return {x:r.left+(puntoSocial.x+1)*r.width/2,y:r.top+(1-puntoSocial.y)*r.height/2,visible:explorando&&cercania<.08&&Math.abs(puntoSocial.x)<.95&&Math.abs(puntoSocial.y)<.95&&Math.abs(puntoSocial.z)<1};
    },
    filtrar(tipo){
      if(!['todo','arboles','construcciones','flora','fauna'].includes(tipo))return;
      filtroElementos=tipo;solicitarFauna=tipo==='fauna';
      if(tipo!=='fauna'&&tipo!=='todo'&&tipo!=='arboles')cerrarArbol();
      bosques?.filtrar(tipo);habitat?.filtrar(tipo);floraMapa?.filtrar(tipo);
      viviendasMapa?.filtrar(tipo);
      window.dispatchEvent(new CustomEvent('musuq:estado',{detail:this.estado()}));
    },
    salir:()=>cambiarModo(false)
  };
  floraMapa=window.MUSUQ_FLORA?.crear({escena,lienzo,posicion:posicionSocial,alAbrir:(planta)=>{
    viviendasMapa?.cerrar(true);
    habitat?.cerrar();
    if(!arbolFoco)plantaPendiente={firma:null,estable:0};
  },sueloCerca:(x,z)=>{const i=celdaEn(x,-z);return i===undefined?null:sueloCelda(i);}})||null;
  viviendasMapa=window.MUSUQ_VIVIENDAS?.crear({escena,lienzo,alAbrir:()=>{floraMapa?.cerrar();habitat?.cerrar();plantaPendiente=null;solicitarFauna=false;},suelo:(x,z)=>{const i=celdaEn(x,-z);return i===undefined?null:sueloCelda(i);}})||null;
