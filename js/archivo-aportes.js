...
(() => {
  ...
  function refresh(){current=doc();selection=null;selectionButton.hidden=true;closeThread();if(!current)return;texts.clear();fields.forEach(id=>texts.set(id,current.categoria&&id!=='detalle-contexto'?'':$(id).textContent));uploader.replaceChildren();if(!current.categoria)uploader.append(avatar(current.avatar||1907,Math.max(0,usuarios.indexOf(current.aportante))),el('span','','Aportado por '),el('strong','',current.aportante||'@archivo.compartido'));seedComments();drawAnnotations();}
  ...
  window.MUSUQ_ARCHIVO_APORTES={montar(a){api=a;},cambiarVista(){selectionButton.hidden=true;closeThread();current=null;},actualizar(){if(doc()?.id!==current?.id)refresh();}};
})();
